...
class DBInterface:
    ...
    def __init__(self, user, password, database_name, host, port):
        ...
        if not sqlalchemy.inspect(self.engine).has_table(model.Payment.__tablename__):
    ...
    def get_all_payments(self) -> list[dict]:
        ...
        with self.open_session(self.global_session) as session:
            payments = session.query(model.Payment).all()
    ...
    def get_chat_payments(self, chat_id: str) -> list[dict]:
        ...
        out = list()
        with self.open_session(self.global_session) as session:
            chat_payments_query = session.query(model.Payment).filter(model.Payment.chat_id == chat_id).all()
    ...
    def get_chat_groups(self, chat_id: str) -> list[str]:
        ...
        with self.open_session(self.global_session) as session:
            chat_payments = session.query(model.Payment).filter(model.Payment.chat_id == chat_id).all()
    ...
    def get_chat_group_payments(self, chat_id: str, group_name: str) -> Optional[dict]:
        ...
        with self.open_session(self.global_session) as session:
            chat_group_payments = session.query(model.Payment).filter(
    ...
    def add_chat_group(self, chat_id: str, group_name: str):
        ...
        if group_name not in self.get_chat_groups(chat_id=chat_id):
            with self.open_session(self.global_session) as session:
                new_chat_payments_object = model.Payment(chat_id=chat_id, group_name=group_name, payments=[])
